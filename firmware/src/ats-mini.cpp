...
// Forward declaration for interrupt handler (place in IRAM for ISR safety)
...
void setup()
{
  ets_printf("ETS: Entering setup()\n");

  // Enable serial port
  Serial.begin(115200);
  ets_printf("ETS: Serial.begin done\n");
  ...
  ledcSetup(LCD_BL_CH, 16000, 8); // Channel, frequency, resolution
  ledcAttachPin(PIN_LCD_BL, LCD_BL_CH);
  ledcWrite(LCD_BL_CH, 0); // Default value 0%
...
// interrupt. If you do not place the ISR in IRAM, the system may reboot
// during attachInterrupt call. IRAM_ATTR ensures this function resides in
// instruction RAM.
//
IRAM_ATTR void rotaryEncoder()
{
...
bool doSeek(int16_t enc, int16_t enca)
{
  ...
  else if (seekMode() == SEEK_SCHEDULE && enc)
  {
    ...
    size_t offset = -1;
    const StationSchedule *schedule = enc > 0 ? eibiNext(currentFrequency + currentBFO / 1000, hour, minute, &offset) : eibiPrev(currentFrequency + currentBFO / 1000, hour, minute, &offset);
...
bool doTune(int16_t enc)
{
  ...
  if (isSSB())
  {
    ...
    step = !stepAdjust ? step : enc > 0 ? step - stepAdjust
                                        : stepAdjust;
