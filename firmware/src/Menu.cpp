...
{
  v += enc;
  v = v > vMax ? vMin + (v - vMax - 1) % (vMax - vMin + 1) : v < vMin ? vMax - (vMin - v - 1) % (vMax - vMin + 1)
                                                                      : v;
  return (v);
}
...
bool doSideBar(uint16_t cmd, int16_t enc, int16_t enca)
{
  ...
  case CMD_MENU:
    doMenu(scrollDirection * enc);
    break;
  case CMD_MODE:
    doMode(scrollDirection * enc);
    break;
  case CMD_STEP:
    doStep(scrollDirection * enc);
    break;
  case CMD_AGC:
    doAgc(enc);
    break;
  case CMD_BANDWIDTH:
    doBandwidth(scrollDirection * enc);
    break;
  case CMD_VOLUME:
    doVolume(enca);
    break;
  case CMD_SOFTMUTE:
    ...
    break;
  case CMD_BAND:
    doBand(scrollDirection * enc);
    break;
  case CMD_AVC:
    doAvc(enc);
    break;
  case CMD_FM_REGION:
    doFmRegion(scrollDirection * enc);
    break;
  case CMD_SETTINGS:
    doSettings(scrollDirection * enc);
    break;
  case CMD_BRT:
    doBrt(enca);
    break;
  case CMD_CAL:
    ...
    break;
  case CMD_THEME:
    doTheme(scrollDirection * enc);
    break;
  case CMD_UI:
    doUILayout(scrollDirection * enc);
    break;
  case CMD_RDS:
    doRDSMode(scrollDirection * enc);
    break;
  case CMD_MEMORY:
    doMemory(scrollDirection * enca);
    break;
  case CMD_SLEEP:
    doSleep(enca);
    break;
  case CMD_SLEEPMODE:
    doSleepMode(scrollDirection * enc);
    break;
  case CMD_USBMODE:
    doUSBMode(scrollDirection * enc);
    break;
  case CMD_BLEMODE:
    doBleMode(scrollDirection * enc);
    break;
  case CMD_WIFIMODE:
    doWiFiMode(scrollDirection * enc);
    break;
  case CMD_ZOOM:
    doZoom(enc);
    break;
  case CMD_SCROLL:
    ...
    break;
  case CMD_UTCOFFSET:
    doUTCOffset(scrollDirection * enc);
    break;
  case CMD_SQUELCH:
    doSquelch(enca);
    break;
