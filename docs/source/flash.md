...
3. Run **just one** of the two following commands (the first one uses three separate firmware files, the second one uses the single merged firmware file):
   ```shell
   uvx --from esptool esptool --chip esp32s3 --port SERIAL_PORT --baud 921600 --before default-reset --after hard-reset write-flash -z --flash-mode keep --flash-freq keep --flash-size keep 0x0 bootloader.bin 0x8000 partitions.bin 0x10000 firmware.bin

   # OR

   uvx --from esptool esptool --chip esp32s3 --port SERIAL_PORT --baud 921600 --before default-reset --after hard-reset write-flash -z --flash-mode keep --flash-freq keep --flash-size keep 0x0 firmware.merged.bin
   ```
