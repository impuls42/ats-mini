...
float batteryMonitor()
{
  ...
  {
  case 0:
    if (batteryVolts > (BATT_SOC_LEVEL1 + BATT_SOC_HYST_2))
      batteryState = 1; // State 0 > 1
    break;
  case 1:
    if (batteryVolts > (BATT_SOC_LEVEL2 + BATT_SOC_HYST_2))
      batteryState = 2; // State 1 > 2
    else if (batteryVolts < (BATT_SOC_LEVEL1 - BATT_SOC_HYST_2))
      batteryState = 0; // State 1 > 0
    break;
  case 2:
    if (batteryVolts > (BATT_SOC_LEVEL3 + BATT_SOC_HYST_2))
      batteryState = 3; // State 2 > 3
    else if (batteryVolts < (BATT_SOC_LEVEL2 - BATT_SOC_HYST_2))
      batteryState = 1; // State 2 > 1
    break;
  case 3:
    if (batteryVolts < (BATT_SOC_LEVEL3 - BATT_SOC_HYST_2))
      batteryState = 2; // State 3 > 2
    break;
  default:
    if (batteryState > 3)
      batteryState = 0; // State (Illegal) > 0
    break;
  }
